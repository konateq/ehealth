...
public class XCAServiceImpl implements XCAServiceInterface {
    ...
    public XCAServiceImpl() {
        ...
        final ServiceLoader<DocumentSearchInterface> serviceLoader = ServiceLoader.load(DocumentSearchInterface.class);
        try {
    ...
    private void prepareEventLogForRetrieve(final EventLog eventLog, final RetrieveDocumentSetRequestType request, final boolean errorsDiscovered,
        ...
        eventLog.setHR_RoleID(Helper.getRoleID(sh));
        eventLog.setSP_UserID(HTTPUtil.getSubjectDN(true));
        eventLog.setPT_ParticipantObjectIDs(Collections.singletonList(Helper.getDocumentEntryPatientIdFromTRCAssertion(sh)));
        eventLog.setAS_AuditSourceId(Constants.COUNTRY_PRINCIPAL_SUBDIVISION);
    ...
    private ExternalIdentifierType makeExternalIdentifier(final String identificationScheme, final String registryObject, final String value, final String name) {
            ...
            try {
                switch (classCodeValue) {
                    ...
                    case PS_CLASSCODE:
                        ...
                        break;
                    case MRO_CLASSCODE:
                        final DocumentAssociation<MroDocumentMetaData> mro = documentSearchService.getMroDocumentList(
                                DocumentFactory.createSearchCriteria().addPatientId(fullPatientId));
                        ...
                        if (mro == null || (mro.getPDFDocumentMetaData() == null && mro.getXMLDocumentMetaData() == null)) {
                            ...
                            final MroDocumentMetaData docPdf = mro.getPDFDocumentMetaData();
                            final MroDocumentMetaData docXml = mro.getXMLDocumentMetaData();
    ...
    private Document transformDocument(final Document doc, final OMElement registryErrorList, final OMElement registryResponseElement, final boolean isTranscode,
                                       final EventLog eventLog) throws DocumentTransformationException {
